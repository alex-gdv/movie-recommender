...
def open_popup_kb():
    ...
    popup = Popup(window, "Knowledge Based Filtering", 250, 250)
    popup_window = popup.get_popup_window()
    # 
    lbl_question = tk.Label(popup_window, text="Do you like " + genres[i] + " movies?")
    lbl_question.pack()
    tk.Button(popup_window, text="Yes", command=yes_btn_pressed).pack()
    tk.Button(popup_window, text="No", command=no_btn_pressed).pack()
    tk.Button(popup_window, text="Get recommendation", command=get_recommendations_kb).pack()
    lbl_recommendations = tk.Label(popup_window, text="", wraplength=250)
...
#https://stackoverflow.com/questions/50944627/image-resize-not-working-on-photoimage-in-tkinter
# https://stackoverflow.com/questions/60103136/canvas-not-filling-available-space-tkinter#:~:text=You%20aren't%20having%20the,fill%20options%20on%20the%20frame.
